...
def piecewise_linear_dt(x, x0, y0, k1):
    ...
    out = np.piecewise(x, [x < x0, x >= x0], [lambda x: y0, lambda x: k1*(x-x0) + y0])
    return out

def piecewise_linear_tw(x, x0, y1, k1):
    """Make piecewise linear model for transitional to wet regimes transition"""
    out = np.piecewise(x, [x < x0, x >= x0], [lambda x: k1*(x-x0) + y1, lambda x: y1])
    return out
...
class LACR:
    ...
    def fit_piecewise_linear_dtw(self):
        ...
        slope_dt = self.fit_piecewise_linear_dt()[2]
        slope_tw = self.fit_piecewise_linear_tw()[2]
        slope = np.nanmean([slope_dt, slope_tw])
        diff = crit - wilt
        y0_ = np.percentile(self.y, 20)
        if diff > 0:
            try:
                p, e = optimize.curve_fit(piecewise_linear_dtw, self.x_std, self.y, p0=[wilt, crit, y0_, slope])  # 0.01
            except:
    ...
    def predicted_flat(self):
        ...
        xd = np.linspace(self.x_std.min(), self.x_std.max(), len(self.x_std))
        yd = np.array([flat[0] for i in range(len(self.x_std))])
        df = pd.DataFrame(data={'y': yd}, index=xd)
        out = df
        return out
    ...
    def predicted_lr(self):
        ...
        xd = np.linspace(self.x_std.min(), self.x_std.max(), len(self.x_std))
        yd = xd * lr[1] + lr[0]
        #out = np.array([x * lr[1] + lr[0] for x in self.x_std])
        df = pd.DataFrame(data={'y': yd}, index=xd)
        df = df[(df.y > self.y.min()) & (df.y < self.y.max())]
        out = df #.values
        return out
    ...
    def predicted_dt(self):
        ...
        xd = np.linspace(self.x_std.min(), self.x_std.max(), len(self.x_std))
        yd = piecewise_linear_dt(xd, *fit_dt)
        df = pd.DataFrame(data={'y': yd}, index=xd)
        df = df[(df.y > self.y.min()) & (df.y < self.y.max())]
        out = df #.values  # out[(out >= 0) & (out <= self.y.max())]
        return out
    ...
    def predicted_tw(self):
        ...
        xd = np.linspace(self.x_std.min(), self.x_std.max(), len(self.x_std))
        yd = piecewise_linear_tw(xd, *fit_tw)
        df = pd.DataFrame(data={'y': yd}, index=xd)
        df = df[(df.y > self.y.min()) & (df.y < self.y.max())]
        out = df #.values  # out[(out >= 0) & (out <= self.y.max())]
        return out
    ...
    def predicted_dtw(self):
        ...
        xd = np.linspace(self.x_std.min(), self.x_std.max(), len(self.x_std))
        yd = piecewise_linear_dtw(xd, *fit_dtw)
        df = pd.DataFrame(data={'y': yd}, index=xd)
        df = df[(df.y > self.y.min()) & (df.y < self.y.max())]
        out = df #.values  # out[(out >= 0) & (out <= self.y.max())]
        return out
    ...
    def residuals_flat(self):
        ...
        mod = self.fit_flat_model()
        ymod = np.array([mod[0] for x in self.x_std])
        out = self.y - ymod
        return out

    def residuals_lr(self):
        """Compute residuals of the linear model"""
        mod = self.fit_linear_model()
        ymod = np.array([mod[0] + x * mod[1] for x in self.x_std])
        out = self.y - ymod
        return out

    def residuals_dt(self):
        """Compute residuals of the dry-to-transitional model"""
        mod = self.fit_piecewise_linear_dt()
        ymod = piecewise_linear_dt(self.x_std, mod[0], mod[1], mod[2])
        out = self.y - ymod
        return out

    def residuals_tw(self):
        """Compute residuals of the transitional-to-wet model"""
        mod = self.fit_piecewise_linear_tw()
        ymod = piecewise_linear_tw(self.x_std, mod[0], mod[1], mod[2])
        out = self.y - ymod
        return out

    def residuals_dtw(self):
        """Compute residuals of the dry-to-transitional-to-wet model"""
        mod = self.fit_piecewise_linear_dtw()
        ymod = piecewise_linear_dtw(self.x_std, mod[0], mod[1], mod[2], mod[3])
        out = self.y - ymod
        return out
